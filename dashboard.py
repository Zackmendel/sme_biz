...
def get_sales_table(b_id):
    try:
        response = supabase.table('sales').select('*').eq("business_id", b_id).execute()
...
def get_product_list(b_id):
    try:
        response = supabase.table("products").select("*").eq("business_id", b_id).execute()
        return response.data if response.data else []
    except Exception as e:
...
def get_purchases_table():
    try:
        response = supabase.table("purchases").select("*").eq("business_id", business_uuid).execute()
        return response.data if response.data else []
    except Exception as e:
...
# Fetch raw data
sales_data = get_sales_table(business_uuid)
products_data = get_product_list(business_uuid)

# Convert to DataFrames safely
df_sales = pd.DataFrame(sales_data)
df_purchases = pd.DataFrame(get_purchases_table())
...
top_product = df_sales['total'].groupby(df_sales['item_name']).sum().idxmax()
...
col_4.metric(label="Profit Margin", value=f"{profit/total_sales*100:,.2f}%")
...
# # Sales vs Expenses
# x_date = df_sales["created_at"]

# y_sales = df_sales["total"].values
# y_expenses = df_purchases["total"].values

# # Create a Plotly Express figure
# fig = px.line(df_sales, x_date, y_sales, title="Sales vs Expenses")
